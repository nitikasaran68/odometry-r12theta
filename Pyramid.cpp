...
Pyramid::Pyramid(frame* prevframe,frame* currentframe,float* pose,depthMap* currDepthMap){
    ...
    steepestDescent=Mat(prev_frame->no_nonZeroDepthPts,2,CV_32FC1);//steepest descent matrix (Nx6)
...
void Pyramid::calculateSteepestDescent()
{
    ...
    Mat jacobian_top(prev_frame->no_nonZeroDepthPts,2, CV_32FC1); //top row of jacobian correspoding to x (Nx6)
    Mat jacobian_bottom(prev_frame->no_nonZeroDepthPts,2, CV_32FC1); //bottom row of jacobian corresponding to y (Nx6)
...
void Pyramid::calculateHessianInv()
{
    ...
    Mat hessian = Mat::zeros(2, 2, CV_32FC1);
    Mat new_temp=temp.t();
    ...
    for(int y=0; y<prev_frame->no_nonZeroDepthPts; y++ )
    {
        ...
        Mat temp_T= (Mat_<float>(2,1)<<new_temp_ptr[0], new_temp_ptr[1],new_temp_ptr[2] );
        Mat steep_T=(Mat_<float>(1,2)<<steep_ptr[0], steep_ptr[1], steep_ptr[2]);
...
void Pyramid::updatePose()
{
    ...
    //this is checked in getImagePoseEstimation when function returns
        ...
        for (int x=0; x<prev_frame->currentCols; x++)
        {
            ...
            if (isnan(weightptr[idx]))
            {
               printf("\n\nWeights:%f , wh: %f, wp: %f, weighted_rp: %f, rp: %f, residualptr[idx]: %f, s: %f, drpdd: %f; ",weightptr[idx], wh, w_p, weighted_rp, rp, residualptr[idx], s, drpdd);
                ...
                for(int k=0; k<prev_frame->no_nonZeroDepthPts; k++)
                {
