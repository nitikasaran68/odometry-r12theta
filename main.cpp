...
//give path to samplefile.txt as command line argument
int main(int argc, char *argv[])
{
    ...
    if (!my_file.is_open()) // check if we succeeded with sample file
    ...
    {
    Mat temp_im;
    for(int m=1;m<util::BATCH_START_ID;m++)
    {
        bgr_capture>>temp_im;
        cout<<"\nSkipping frame: "<<m;
        waitKey(5);
    ...
    for (int frame_counter = 1; frame_counter <= max_frame_counter; frame_counter++)
    {
...
//            printFlags();
        ...
        util::measureTime time_main; //to measure execution time
