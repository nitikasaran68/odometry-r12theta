...
void globalOptimize::resetArrayElement(int arrayId)
{
    ...
    //first reset similarity element
    //similarityAverageLoop.resetArrayElement(arrayId);
...
//    {
//        printf("\nChecking exit condition from Loop Closure...");
//        bool toExit=checkExitCondition();
//        if(toExit)
//        {
//            printf("\nDetected exit condition in loop closure");
...
//    }
 ...
 //   frame* loopFrameArray[currentArrayId].this_frame=malloc(sizeof(frame));
    
    
  //  loopFrameArray[currentArrayId].this_frame=currentframe;
  ...
  //  loopFrameArray[currentArrayId].new_frame=new frame(currentframe->image, currentframe->frameId);
    
    //loopFrameArray[currentArrayId]=currentLoopFrame;
  //  memcpy(loopFrameArray[currentArrayId].this_frame, currentframe, sizeof(frame));
  //  memcpy(loopFrameArray[currentArrayId].this_currentDepthMap,currentDepthMap, sizeof(depthMap));
   ...
   //scaling depth map with the global scale
   // printf("\nCalling scaleDepthMap for frame id: %d", loopFrameArray[currentArrayId].frameId);
   // loopFrameArray[currentArrayId].this_currentDepthMap->scaleDepthMap();
    
    
    
    // printf("\n\nAFTER Pushing frame with id: %d and depth map with non-zero points: %d", loopFrameArray[currentArrayId].this_frame->frameId, loopFrameArray[currentArrayId].this_frame->no_nonZeroDepthPts);
    
   // loopFrameArray[currentArrayId].this_frame->updationOnPyrChange(3);
   // printf(" , After func: %d \n ", loopFrameArray[currentArrayId].this_frame->no_nonZeroDepthPts );
    
    
    if(util::FLAG_DO_SIMILARITY_AVERAGE)
    {
        //setting id where similarity average parameters must be finalised
        setSimAvgCenter();
    }
...
bool globalOptimize::findMatch(frame *currentframe, bool strayFlag)
{
    ...
    if(lastTestedLoopClosureArrayId==-1)
    {
        ...
        //i=match_window_end; //starts from end of window, most recent
...
//        if(strayFlag)
//        {   printf(" RESULT: Match value : %f", matchValue);
//            imshow("Potential Matched frame", loopFrameArray[i].image);
...
void globalOptimize::setSimAvgCenter()
{
    ...
    if (!loopFrameArray[util::MAX_LOOP_ARRAY_LENGTH+1].isValid)
    {
        simavg_center=-1;
        return;
    }
    
    simavg_center=currentArrayId-(util::MAX_LOOP_ARRAY_LENGTH)-1;
    
    if (simavg_center<0)
    {
        simavg_center+= util::MAX_LOOP_ARRAY_LENGTH_SCALE_AVG;
    }
    
    printf("\nsimavg center: %d, current array id: %d", simavg_center, currentArrayId);
    ...
    if( /*(frame_counter%util::KEYFRAME_PROPAGATE_INTERVAL==0) && */ waitFrameCount==0)
    {
        ...
        do
        {
            ...
            if(matchFound==true)
            {
                ...
                /* write pose??!! not updated to text file!!!, if required can be done in main!
                seeds_num= loopFrameArray[loopClosureArrayId].this_currentDepthMap->calculate_no_of_Seeds();
                ...
                if(util::FLAG_WRITE_MATCH_POSES)
                {
                    ...
                    if(match_file.is_open())
                    {
                        match_file<<testFrame->frameId<<" "<<loopFrameArray[loopClosureArrayId].this_frame->frameId<<" "<<testFrame->poseWrtOrigin[0]<<" "<<testFrame->poseWrtOrigin[1]<<" "<<testFrame->poseWrtOrigin[2]<<" "<<testFrame->poseWrtOrigin[3]<<" "<<testFrame->poseWrtOrigin[4]<<" "<<testFrame->poseWrtOrigin[5]<<" "<<loopFrameArray[loopClosureArrayId].this_frame->rescaleFactor<<" "<<seeds_num<<" "<<matchValue<<" "<<rms_error<<" "<<relative_view_angle<<"\n";
                    }
                }
