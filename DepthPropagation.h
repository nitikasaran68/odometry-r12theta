...
#ifndef __odometry__DepthPropagation__
#define __odometry__DepthPropagation__
...
public:
    ...
    //vector<depthhypothesis> depthHypothesisVec; ////////// TO BE INITIALIZED FOR EVERY KEYFRAME
    //vector<depthhypothesis>* depthHypothesisVecPtr;
    
    depthhypothesis currentDepthHypothesis[5000000]; //array of depth hyposthesis, size is equal to img size
    depthhypothesis otherDepthHypothesis[5000000]; //used as a temporary array for swapping
    ...
    //vector<frame*>* frameptrvectorptr; //points to vector of frame pointers
    ...
    //float deptharrpyr4[(util::ORIG_ROWS>>4)*(util::ORIG_COLS>>4)];
    ...
    //float depthvararrpyr4[(util::ORIG_ROWS>>4)*(util::ORIG_COLS>>4)];
    ...
    void refineDepth( vector<frame*> frameptrvector);
...
#endif /* defined(__odometry__DepthPropagation__) */
