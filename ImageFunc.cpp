...
//        prev_frame->saveArrayAsText(currDepthMap->depthvararrpyr3, "DepthVarArr_pyr3", util::SAVED_MATS_PATH, 3);
    ...
    pose[2]=tminus1_prev_frame->poseWrtOrigin[2];
    pose[3]=tminus1_prev_frame->poseWrtOrigin[3];
    pose[4]=tminus1_prev_frame->poseWrtOrigin[4];
    pose[5]=tminus1_prev_frame->poseWrtOrigin[5];
    */
...
for (int level_counter=(util::MAX_PYRAMID_LEVEL-1); level_counter>=0; level_counter--)
{
   ...
   // printf("\nAt Pyramid Level: %d , Max Iteration: %d ",level_counter,disp_iter_counter+1);
    ...
    //printf("\nPose of curr id: %d with prev id: %d and from loop closure: %d, TIME: %f", current_frame->frameId, prev_frame->frameId, fromLoopClosure, elapsed);
